...
test("Test id:1 for FR1", async () => {
  ...
  await Axios.post(
    ...
    .then(function(response) {
      expect(response.message).toBe("Sent to message bus");
    })
    .catch(function(error) {
...
// Negative test for FR1
test("Test id:2 for FR1", async () => {
  let auth_token = "";
  await Axios.post("http://localhost:9000/login", {
    name: "Albin",
    password: "password123"
  ...
  let eiffelDataObj = {
    meta: {
      type: "EEEiffelConfidenceLevelModifiedEvent",
      version: "5.0.0",
      time: 1234567890,
      id: "ff6c8ce5-e21b-4eb3-970b-d665dad9e4f8"
    },
    data: {
      name: "stable",
      value: "SUCCESS",
